...
def test_get_dataset_full(
    sub_sampled_low_res_config_path: str = "../../config/dataset/sub_sampled_low_res.yaml",
):
    ...
    dataset_cfg: DictConfig = OmegaConf.create(
        {
            ...
            "data_path": data_path,
            "dataset_testing_fractions": {"quick": 0.01, "reduced": 0.1, "full": 1.0},
            "group_method": False,
    ...
    dataset = data_preparation.select_data.get_dataset(
        dataset_cfg, mode, dataset_testing_type
    )
    assert isinstance(
        dataset, data_preparation.sub_sampled_low_res.SubSampledLowResDataset
    )
...
def test_get_dataset_reduced(
    ...
    dataset_cfg: DictConfig = OmegaConf.create(
        {
            "dataset_name": "subsampled_low_res",
            "data_path": data_path,
            "dataset_testing_fractions": {"quick": 0.1, "reduced": 0.5, "full": 1.0},
            "group_method": False,
            "remove_high_altitude_specific_humidity_levels": False,
        }
    )
    ...
    assert (
        len(dataset) == 1920
    )  # int(config.dataset_testing_fractions.unit_test * dataset_cfg.dataset_testing_fractions.reduced)


def test_get_dataset_quick(
    sub_sampled_low_res_config_path: str = "../../config/dataset/sub_sampled_low_res.yaml",
):
    base_dir = Path(__file__).resolve().parents[1]
    data_path = os.path.join(base_dir, "unit_test_sets", "sub_sampled_low_res/")
    ...
    assert (
        len(dataset) == 384
    )  # int(config.dataset_testing_fractions.unit_test * dataset_cfg.dataset_testing_fractions.quick)


def test_get_datasets_sub_sampled_low_res(
    sub_sampled_low_res_config_path: str = "../../config/dataset/sub_sampled_low_res.yaml",
):
    base_dir = Path(__file__).resolve().parents[1]
    data_path = os.path.join(base_dir, "unit_test_sets", "sub_sampled_low_res/")
    ...
    dataset_testing_type = "quick"
    trainset, valset, testset = data_preparation.select_data.get_all_datasets(
        dataset_cfg, dataset_testing_type
    )
...
def test_sample_data_based_on_testing_type_fractions():
    ...
    dataset_testing_fractions = OmegaConf.create(
        {"quick": 0.1, "reduced": 0.5, "full": 1.0}
    )

    # Test quick
    input_quick, target_quick = (
        data_preparation.select_data.sample_data_based_on_testing_type(
...
def test_select_first_n_samples_single():
    ...
    input_selected, target_selected = (
        data_preparation.select_data.select_first_n_samples(data, n_samples)
    )
...
def test_get_climsim_from_raw():
    ...
    dataset = data_preparation.select_data.get_dataset(
        dataset_config, mode, dataset_testing_type
    )
    assert isinstance(dataset, data_preparation.ClimSimFromRawDataset)


def test_get_climsim_from_raw_with_test():
    base_dir = Path(__file__).resolve().parents[1]
    data_path = os.path.join(
        base_dir, "unit_test_sets", "dummy_low_res_climsim/", "filename_testing"
    )
    ...
    testset = data_preparation.select_data.get_dataset(
        dataset_config,
        "test",
    ...
    assert isinstance(testset, data_preparation.ClimSimFromRawDataset)


def test_get_datasets_from_raw():
    base_dir = Path(__file__).resolve().parents[1]
    data_path = os.path.join(
        base_dir, "unit_test_sets", "dummy_low_res_climsim/", "filename_testing"
    )
    ...
    train_dataset, val_dataset, test_dataset = (
        data_preparation.select_data.get_all_datasets(
            dataset_config,
            dataset_testing_type,
            model=None,
        )
