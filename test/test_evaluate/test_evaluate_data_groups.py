...
def test_evaluate_data_group_multivariate():
...
def test_evaluate_data_group_marginals():
    ...
    results = evaluate.evaluate_data_group(cfg, X_dataset, Y_dataset, ["marginals"])

    dist = results["marginals"]['marginal_distances']
    assert len(dist) == X.shape[1]
