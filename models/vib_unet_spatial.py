...
class VIBUNetSpatial(nn.Module):
    def __init__(self, beta: float):
        """
        Initializes the VIBUNet model.
        """
        super().__init__()
        self.name = "vib_unet_spatial"
        self.beta = beta
    ...
    def step(self, output, y, log, loss_metric, stage=None):
        ...
        standard_loss_nh = loss_metric(y_hat_nh, y_nh)
        standard_loss_sh = loss_metric(y_hat_sh, y_sh)
        standard_loss = standard_loss_nh + standard_loss_sh
        log(f"{stage}/loss", standard_loss)

        logvar_diff = logvar_nh - logvar_sh  # equals log(var_q/var_p)
        kl_per_elem = 0.5 * (
            -logvar_diff
            + torch.exp(logvar_diff)  # = var_q / var_p
            + (mu_nh - mu_sh).pow(2) * torch.exp(-logvar_sh)  # divide by var_p
            - 1.0
        )
