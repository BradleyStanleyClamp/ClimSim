...
#         if use_skip:
#             self.skip = nn.Conv1d(out_channels, out_channels, kernel_size=3, padding=1)
...
#         self.enc['8_block1'] = UNetBlockNoAttention(in_channels=256, out_channels=256, use_skip=False, group_norm_groups=32, activation=nn.SiLU())

#         # Base layer
#         self.base = UNetBlockNoAttention(in_channels=256, out_channels=256, use_skip=False, group_norm_groups=32, activation=nn.SiLU())
...
#         assert x.shape == (x.shape[0], 256, 32), f"After 32_block1, shape must be (batch_size, 256, 32), but got {x.shape}"
        
#         x, skip_16 = self.enc['16_down'](x)
#         assert x.shape == (x.shape[0], 256, 16), f"After 16_down, shape must be (batch_size, 256, 16), but got {x.shape}"
#         x, skip_16_block0 = self.enc['16_block0'](x)
#         assert x.shape == (x.shape[0], 256, 16), f"After 16_block0, shape must be (batch_size, 256, 16), but got {x.shape}"
#         x = self.enc['16_block1'](x)
#         assert x.shape == (x.shape[0], 256, 16), f"After 16_block1, shape must be (batch_size, 256, 16), but got {x.shape}"
        
#         x, skip_8 = self.enc['8_down'](x)
#         assert x.shape == (x.shape[0], 256, 8), f"After 8_down, shape must be (batch_size, 256, 8), but got {x.shape}"
#         x, skip_8_block0 = self.enc['8_block0'](x)
#         assert x.shape == (x.shape[0], 256, 8), f"After 8_block0, shape must be (batch_size, 256, 8), but got {x.shape}"
#         x = self.enc['8_block1'](x)
#         assert x.shape == (x.shape[0], 256, 8), f"After 8_block1, shape must be (batch_size, 256, 8), but got {x.shape}"
        
        
#         x = self.base(x)
#         assert x.shape == (x.shape[0], 256, 8), f"After base, shape must be (batch_size, 256, 8), but got {x.shape}"
