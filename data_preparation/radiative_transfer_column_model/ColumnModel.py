"""
Radiative Transfer Column Model from Brian Rose's Climate Modeling class https://www.atmos.albany.edu/facstaff/brose/classes/ENV480_Spring2014/styled-5/code-2/index.html
...
import numpy as np
from . import ClimateUtils as clim
...
class column:
    ...
    def __str__(self):
        return (
            "Instance of column class with surface temperature "
            + str(self.Ts)
            + " K, "
            + str(self.params["num_levels"])
    ...
    def __init__(self, p=None, params=dict(nul=None)):
        ...
        params.setdefault("timestep", 1.0 * clim.seconds_per_day)
        ...
        if p is None:
            self.dp = (
                clim.ps / self.params["num_levels"]
            )  #  pressure interval in hPa or mb
            self.p = np.linspace(
                clim.ps - self.dp / 2, self.dp / 2, params["num_levels"]
            ...
            N = self.p.size
            self.pbounds = np.concatenate(
                ([clim.ps], (self.p[0 : N - 1] + self.p[1:N]) / 2, [0.0])
            )
            self.dp = np.flipud(np.diff(np.flipud(self.pbounds)))
            self.params["num_levels"] = N

        self.Ts = 288.0  # initial surface temperature
        self.Tatm = np.linspace(
            self.Ts - 10.0, 200.0, params["num_levels"]
        )  # intitial temperature
        self.c_atm = (
            clim.cp * self.dp * clim.mb_to_Pa / clim.g
        )  #  heat capacity of atmospheric layers
        self.c_sfc = (
            clim.cw * clim.rho_w * params["water_depth"]
        )  #  heat capacity of surface in J / m**2 / K
        self.set_LW_emissivity()
        sw_eps = (
            self.eps * 0.001
        )  # example: set shortwave absorptivity to a small fraction of longwave emissivity
        self.set_SW_absorptivity(sw_eps)
    ...
    def set_LW_emissivity(self, eps=None):
        ...
        if eps is None:
            ...
            self.eps = 2.0 / (
                1 + 2.0 * clim.g / self.params["abs_coeff"] / (self.dp * clim.mb_to_Pa)
            )
        elif np.isscalar(eps) or eps.size == self.params["num_levels"]:
            self.eps = eps
        else:
            raise ValueError(
                "eps must be scalar or have exactly "
                + self.param["num_levels"]
                + " elements."
            )
    ...
    def set_SW_absorptivity(self, eps=None):
        ...
        if eps is None:
            # print('Setting default shortwave absorptivity to zero')
            # default is no shortwave absorption
            self.SWtrans = Transmissivity(np.zeros_like(self.p))
        elif np.isscalar(eps):
            # print('Setting uniform shortwave absorptivity to ', eps)
            ...
            # print('Setting shortwave absorptivity to specified profile')
    ...
    def Longwave_Heating(self):
        ...
        self.emit_sfc = clim.sigma * self.Ts**4.0
        self.emit_atm = eps * clim.sigma * self.Tatm**4.0
        ...
        self.LW_absorbed_atm = (
            incident_fromatm + incident_fromsfc
        ) * eps - 2 * self.emit_atm
    ...
    def Shortwave_Heating(self):
        ...
        # print(f' SW_incident_fromabove: {SW_incident_fromabove}')
        # print(f' SW_incident_frombelow: {SW_incident_frombelow}')
        # print(f' self.SWtrans.absorb: {self.SWtrans.absorb}')
        self.SW_absorbed_atm = (
            SW_incident_fromabove + SW_incident_frombelow
        ) * self.SWtrans.absorb
    ...
    def Rad_Temperature_Tendency(self):
        ...
        self.rad_temp_tendency_sfc = (
            self.rad_heating_sfc * self.params["timestep"] / self.c_sfc
        )
        self.rad_temp_tendency_atm = (
            self.rad_heating_atm * self.params["timestep"] / self.c_atm
        )
    ...
    def Convective_Adjustment(self, lapserate=6.5):
        ...
        if lapserate is "DALR":
            lapserate = clim.g / clim.cp * 1.0e3
        try:
            alpha = clim.Rd / clim.g * lapserate / 1.0e3
    ...
    def Step_Forward(self, num_steps=1):
        ...
        for n in range(num_steps):
            ...
            if self.params["adj_lapse_rate"] is not None:
                ...
                Tadj = self.Convective_Adjustment(
    ...
    def update_time(self):
        ...
        self.steps += 1
        self.days_elapsed += (
            self.params["timestep"] / clim.seconds_per_day
...
class Transmissivity:
    def __init__(self, absorb):
        ...
        self.surf2atm = np.concatenate(([1.0], np.cumprod(self.trans[: N - 1])))
        # the transmissivity between layer k and space
        self.atm2space = np.flipud(
            np.cumprod(np.concatenate(([1.0], np.flipud(self.trans[1:N]))))
        )
